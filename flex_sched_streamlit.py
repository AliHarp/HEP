...
sessions_per_weekday = {'Monday': 3, 'Tuesday': 3, 'Wednesday': 3, 'Thursday': 3, 'Friday': 3, 'Saturday': 1, 'Sunday': 1}
allocation = {'Monday': ['2P_or_1R', '2P_or_1R', '1P'], 'Tuesday': ['2P_or_1R', '2P_or_1R','1P'], 'Wednesday': ['2P_or_1R', '2P_or_1R','1P'], 
              'Thursday': ['2P_or_1R', '2P_or_1R', '1P'], 'Friday': ['2P_or_1R', '2P_or_1R', '1P'], 'Saturday': ['2P_or_1R'], 'Sunday': ['1P']}
...
st.sidebar.markdown("For each weekday, please select:")
...
for day,session,session_key, allocate, allocate_key, theatre, theatre_key, keylist, indices in zip(weekday, sessions_per_weekday.values(), 
	...
	with st.expander(f"View scheduling details for {day}"):
		...
		st.write(f"Please select the surgery allocation for each session.")
		st.write(f"You may allocate {selected_sessions} of the following: ")
		...
		#	st.write("You have set the session number to 0, no selection available")
			
		remaining = selected_sessions - len(selected_allocations)
		if (len(selected_allocations) < selected_sessions):
			st.write(f"You have {remaining} options remaining - all sessions must be allocated")
		...
		st.write("If you have selected 0 sessions per day, the number of theatres in use will also be 0")
		if selected_sessions == 0:
			selected_theatres = 0
			selected_theatres = st.slider("Number of theatres:", 0, 6, 0)
		st.write("Selected theatres:", selected_theatres)
		theatres_per_weekday[theatre_key] = selected_theatres

##########################################################################################
#####################  GENERATE SCHEDULE AND OUTPUT AS CSV ###############################
##########################################################################################
